// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// RestCreatePeerFolderRequest rest create peer folder request
//
// swagger:model restCreatePeerFolderRequest
type RestCreatePeerFolderRequest struct {

	// Path to the folder to be created
	Path string `json:"Path,omitempty"`

	// Restrict listing to a given peer
	PeerAddress string `json:"PeerAddress,omitempty"`
}

// Validate validates this rest create peer folder request
func (m *RestCreatePeerFolderRequest) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this rest create peer folder request based on context it is used
func (m *RestCreatePeerFolderRequest) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *RestCreatePeerFolderRequest) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *RestCreatePeerFolderRequest) UnmarshalBinary(b []byte) error {
	var res RestCreatePeerFolderRequest
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
